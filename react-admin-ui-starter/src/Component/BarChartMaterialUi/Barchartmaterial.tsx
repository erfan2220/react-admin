...
const RoundedBar: FC<{ x: number; y: number; height: number; fill: string }> = (props) => {
...
const BarChartMaterial: FC<Props> = ({ data }) => {
    /*const seriesColors = ['#007BFF', '#18D3CC', '#99CAFF'];*/

    const filter_counters = (vendor: string, type: string) => {
    ...
    const CustomTooltip: FC<{ active?: boolean; payload?: any[]; label?: string }> = ({ active, payload }) => {
    ...
    const dataSet: SiteData[] = [
        { name: 'Nokia', uv: filter_counters("NOKIA", "2G"), pv: filter_counters("NOKIA", "3G"), cv: filter_counters("NOKIA", "4G") },
        { name: 'Ericsson', uv: filter_counters("ERICSSON", "2G"), pv: filter_counters("ERICSSON", "3G"), cv: filter_counters("ERICSSON", "4G") },
        { name: 'Huawei', uv: filter_counters("HUAWEI", "2G"), pv: filter_counters("HUAWEI", "3G"), cv: filter_counters("HUAWEI", "4G") },
    ];
    ...
    const CustomYAxisTick: FC<{ x?: number; y?: number; payload?: any }> = ({ x, y, payload }) => (
        <text
            x={x}
            y={y}
            dy={7}
                ...
                <div className="test_barchart_container">
                    <ResponsiveContainer width="80%" height="100%" >
                        ...
                        >
                            ...
                            <XAxis stroke="#757575" dataKey="name" ticks={customXAxisTicks} tickMargin={10} tickCount={7} padding={{ left: 40 }} fontWeight={600} />
                            <YAxis
                                stroke="#757575"
                            ...
                            <Bar dataKey="uv" fill="#007BFF" shape={<RoundedBar radius={4} />} />
                            <Bar dataKey="pv" fill="#18D3CC" shape={<RoundedBar radius={4} />} />
                            <Bar dataKey="cv" fill="#99CAFF" shape={<RoundedBar radius={4} />} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
