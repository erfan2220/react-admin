...
const Site = () => {
    ...
    useEffect(() => {
        ...
        const promises = [];
        const promise1 = fetchAndCacheData("sites_data_test_cache", `http://10.15.90.87:5001/api/assets/sites_detail_per_city/${cityName}`)
            .then(data => {
    ...
    async function fetchAndCacheData(cacheKey:string, apiUrl:string)
    {
                ...
                <Tabs value={selectedTab} >
                    {siteTabs.map((site) => (
                        <Tab key={site.siteName} label={site.siteName} value={site.siteName}
                             onClick={() => handleTabChange(site.siteName) }/>
                    ))}
                </Tabs>
            </div>
